...
export async function DELETE(request: Request) {
  // (You can add similar A/B testing logic here if needed)
  const body = await request.json();
  const sessionId = body.sessionId as string;
  // For simplicity, using the open-operator provider functionality here
  const operator = await getOperatorProvider();
  // Assuming both providers have an endSession method (not shown here)
  if (operator.endSession) {
    await operator.endSession(sessionId);
  }
